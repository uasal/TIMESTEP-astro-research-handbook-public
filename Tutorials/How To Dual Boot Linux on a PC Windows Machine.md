...
Ubuntu is the most commonly downloaded Linux distro. It also has the most similar interface to Windows. It is a mature flavor of Linux, and its developers, Cannonical, offers users a lot of support and resources. Download [Ubuntu 20.04.3 LTS](https://ubuntu.com/download/desktop/thank-you?version=22.04.1&architecture=amd64).
...
Should you be using a multidrive system, you can also use another of your drives; the procedure is the same. However, if one of your drives is a Solid State Drive (SSD), this is the preferred installation drive as SSDs are fast in comparison to traditional hard drives. SSDs used as hosts for system drives improve performance immensely.
...
Right click on your C Drive (either in the top or bottom portion of the window) and then click on '**Shrink Volume**'. After it queries your drive controller, you'll get a window that looks similar to this:  

![Shrink Drive Volume](/Assets/Images/ShrinkVolume.png).

In the box that reads '**Enter the amount of space to shrink in MBs**', enter the amount of space you've decided to allocate to Linux. Remember, this is in megabytes, so multiple how many GBs you want by 1000. Click '**Shrink**'.
...
Windows has always had a reputation for taking forever to start itself, and this was a Microsoft workaround. But it makes Windows start time faster by not actually shutting down the machine... it freezes its state and pretends to reboot itself. This wrecks havoc on a second OS trying to establish control during boot. By default it is on; let's turn it off.
...
How you proceed from this point depends largely on what Linux distro you are installing, which may differ somewhat from Ubuntu.
...
The installer should recognize that the system it is loading on has Windows installed. This is where the process deviates from a normal install. If all goes well, you will see a message saying '**Install Ubuntu Alongside Windows?**. Select '**Yes**'.
